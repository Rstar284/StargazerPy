...
class Rust(commands.Cog):
    ...
    @commands.command()
    async def rusteval(self, ctx: commands.Context, *, arg):
        """Evaluates Rust code. Exactly equal to https://play.rust-lang.org/ stolen from the rust discord server bot btw"""
        (mode, code) = self.parse_args(arg)
        await self.send_playground(ctx, mode, code.source)

    @commands.command()
    async def rustevalwarn(self, ctx: commands.Context, *, arg):
        """Evaluates Rust code and outputs generated warnings. Exactly equal to https://play.rust-lang.org/ stolen from the rust discord server bot btw"""
        (mode, code) = self.parse_args(arg)
