...
class MemberID(commands.Converter):
    async def convert(self, ctx, argument):
        ...
        if not can_execute_action(ctx, ctx.author, m):
            raise commands.BadArgument('You cannot do this action on this user due to the user being higher than me.')
        return m
...
class NoMuteRole(commands.CommandError):
    def __init__(self):
        super().__init__('This server did not set up a mute role.')
...
class Mod(commands.Cog):
    ...
    async def cog_command_error(self, ctx, error):
        if isinstance(error, commands.BadArgument):
            ...
            if isinstance(original, discord.Forbidden):
                ...
                await ctx.send(f'This thing does not exist: {original.text}')
            elif isinstance(original, discord.HTTPException):
                await ctx.send('Somehow, an unexpected error occurred. Try again later, Mabye?')
    ...
    async def check_raid(self, config, guild_id, member, message):
        ...
        try:
            await member.ban(reason='Auto-ban for spam (strict raid mode ban)')
        except discord.HTTPException:
    ...
    async def multiban(self, ctx, members: commands.Greedy[MemberID], *, reason: ActionReason = None):
        ...
        This only works through banning with user ID.
        ...
        if reason is None:
            reason = f'Multiban done by {ctx.author} (ID: {ctx.author.id})'
        ...
        if not confirm:
            return await ctx.send('Canceling Action...')
    ...
    async def massban(self, ctx, *, args):
        ...
        if not isinstance(ctx.author, discord.Member):
            try:
                ...
                return await ctx.send('Somehow, Discord does not seem to think you are in this server. Yes weird i know, better complain to discord than me.')
        else:
            author = ctx.author
    ...
    async def softban(self, ctx, member: MemberID, *, reason: ActionReason = None):
        ...
        if reason is None:
            reason = f'Softban done by {ctx.author} (ID: {ctx.author.id})'
    ...
    async def tempmute(self, ctx, duration: time.FutureTime, member: discord.Member, *, reason: ActionReason = None):
        ...
        if reason is None:
            reason = f'Tempmute done by {ctx.author} (ID: {ctx.author.id})'
        ...
        await reminder.create_timer(duration.dt, 'tempmute', ctx.guild.id,
                                                                     ctx.author.id,
                                                                     member.id,
                                                                     role_id,
                                                                     created=ctx.message.created_at)
    ...
    async def tempunmute(self, ctx, duration: time.FutureTime, member: discord.Member, *, reason: ActionReason = None):
        ...
        role_id = ctx.guild_config.mute_role_id
        await member.remove_roles(discord.Object(id=role_id), reason=reason)
        await reminder.create_timer(duration.dt, 'tempunmute',ctx.guild.id,
                                                                     ctx.author.id,
                                                                     member.id,
                                                                     role_id,
                                                                     created=ctx.message.created_at)
        await ctx.send(f'unmuted {discord.utils.escape_mentions(str(member))} for {time.format_relative(duration.dt)}.')
